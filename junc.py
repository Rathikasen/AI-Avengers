...
import cv2
import numpy as np
# Vehicle Detector class with lane time calculation
...
class VehicleDetectionScreen(Screen):
    ...
    def update_vehicle_count_display(self):
        ...
        for i, lane_count in enumerate(self.counts):
            self.layout.add_widget(Label(text=f"Lane {i + 1}: {lane_count} vehicles, Time allocated: {int(self.time_allocations[i])} seconds"))
    ...
    def switch_signals(self, dt=None):
        ...
        Clock.schedule_once(self.switch_signals, int(self.time_allocations[self.current_lane]))
